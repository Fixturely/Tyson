...
import { confirmPaymentIntent, createPaymentIntent } from '../../../services/stripe';
...
export async function confirmPaymentIntentController(req: Request, res: Response) {
    try {
        const { id } = req.params;
        ...
        logger.error(`Failed to confirm payment intent: ${err.message}`, { error: err });
